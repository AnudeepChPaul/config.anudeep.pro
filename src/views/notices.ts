...
const WORDING: Record<string, (count: number | null) => Notice> = {
  ...
  retiring: () => ({
    tone: 'done',
    text: 'Marked as retiring, as a draft. Nothing changes for a consumer until it is published.',
  }),
  'retirement-cancelled': () => ({
    tone: 'done',
    text: 'Retirement cancelled, as a draft. Publish it to bring the product back.',
  }),
